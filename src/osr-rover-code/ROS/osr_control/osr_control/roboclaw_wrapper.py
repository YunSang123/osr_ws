...
class RoboclawWrapper(Node):
    ...
    def read_encoder_position(self, address, channel):
        ...
        if channel == "M1":
            ...
            #print("M1_val : {}".format(val))
        elif channel == "M2":
            val = self.rc.ReadEncM2(address)
            #print("M2_val : {}".format(val))
        else:
...
#####################################################################################################
    ...
    def send_velocity_cmd(self, address, channel, target_qpps):
        ...
        # -self.roboclaw_overflow < target_qpps < self.roboclaw_overflow
        #print("address : {}, channel : {}".format(address, channel))
        if channel == "M1":
            # print("M1")
