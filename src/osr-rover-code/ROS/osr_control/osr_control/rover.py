...
class Rover(Node):
    ...
    def cmd_cb(self, twist_msg, intuitive=True):
        ...
        if twist_msg.angular.z and not twist_msg.linear.x:
    ...
    def enc_cb(self, msg):
        ...
        if self.should_calculate_odom and len(self.curr_positions) == 10:
            ...
            dx = 12 * self.curr_twist.twist.linear.x * dt
            dth = self.curr_twist.twist.angular.z * dt
            print("twist.twist.angular.z : {}".format(self.curr_twist.twist.angular.z))
            ...
            self.odometry.twist.covariance[-1] = 0.04 # tmp : 0.04
            ...
            if (current_angle!=0) and (new_angle!=0):
                print("current_angle = {}".format(current_angle * 180/math.pi))
                print("new_angle = {}".format(new_angle * 180/math.pi))
                print("dy = {}".format(10*math.sin(new_angle) * dx))
    ...
    def calculate_corner_positions(self, radius):
        ...
        if radius > 0:
            cmd_msg.left_front_pos = -theta_front_closest       # theta_front_closest
            ...
            print("turn left + radius > 0")
            print("front_left_angle : ", -theta_front_closest)
            print("back_left_angle : ", theta_front_closest)
            print("back_right_angle : ", theta_front_farthest)
            print("front_right_angle : ", -theta_front_farthest)
        else:
            ...
            print("turn right + radius < 0")
            print("front_left_angle : ", theta_front_farthest)
            print("back_left_angle : ", -theta_front_farthest)
            print("back_right_angle : ", -theta_front_closest)
            print("front_right_angle : ", theta_front_closest)
    ...
    def calculate_rotate_in_place_cmd(self, twist):
        ...
        corner_cmd.left_front_pos = math.atan(0.75)
        corner_cmd.left_back_pos = -math.atan(0.85)
        corner_cmd.right_back_pos = math.atan(0.85)
        corner_cmd.right_front_pos = -math.atan(0.75)
    ...
    def forward_kinematics(self):
        ...
        #######################################
        theta_bl = self.curr_positions['corner_left_back'] # - is tmp
        theta_br = self.curr_positions['corner_right_back'] # - is tmp
        #######################################
